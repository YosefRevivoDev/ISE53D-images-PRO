...
class PlaneTest {
    ...
    public void testFindIntersections() {
        ...
        assertEquals(1, plane.findIntersections(new Ray(new Point(0, -2, 0), new Vector(0, 2, 1))).size(),"Ray's line out of plane");

        //TC02: Ray's neither orthogonal nor parallel to the plane and not intersects the plane (0 points)
        assertEquals(null, plane.findIntersections(new Ray(new Point(0, -1, 0), new Vector(-1, -1, 0))),"Ray's line is in plane");
        ...
        assertEquals(null, plane.findIntersections(new Ray(new Point(2, 0, 0), new Vector(1, 0, 0))),"Ray's line is not in the plane");

        // TC12: Ray are not included in the plane (0 points)
        assertEquals(null, plane.findIntersections(new Ray(new Point(2, -1, 0), new Vector(-1, 0, 0))),"Ray's line is in plane");

        // **** Group: Ray's line is orthogonal to the plane

        //TC13 Ray is  orthogonal to the plane before (0 points)
        assertEquals(null, plane.findIntersections(new Ray(new Point(0, 1, 0), new Vector(0, 2, 0))),"the Ray is not orthogonal to the plane");

        //TC14 Ray is  orthogonal to the plane after (0 points)
        assertEquals(null, plane.findIntersections(new Ray(new Point(0, -1, 0), new Vector(0, -2, 0))),"the Ray is not orthogonal to the plane");

        //TC15 Ray is  orthogonal in the plane
        assertEquals(null, plane.findIntersections(new Ray(new Point(2, 0, 1), new Vector(0, 1, 0))),"the Ray is not orthogonal in the plane");
        ...
        assertEquals(null, plane.findIntersections(new Ray(new Point(2, 0, 1), new Vector(0, 2, -1))),"Ray's not start at Q0");
